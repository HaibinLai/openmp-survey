...
static const int step_num = 10000;

// compile rules: g++ -O2 ./bench.cpp -o bench_cpp -fopenmp -std=c++14
...
# pragma omp parallel for simd reduction(+:pe,ke) private ( d, d2, rij, i, j, k ) shared ( f, nd, np, pos, vel )
...
# pragma omp parallel for simd private ( i, j ) shared ( acc, dt, f, nd, np, pos, rmass, vel )
  for ( j = 0; j < np; j++ )
  {
    for ( i = 0; i < nd; i++ )
    {
